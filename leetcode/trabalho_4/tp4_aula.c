	...
	for (int i = 0; i < numsSize; i++){
		for (int j = i+1; j < numsSize; j++){
			for (int k = i+2; k < numsSize; k++){
				for (int l = i+3; l < numsSize; l++){
					
					if (nums[i] + nums[j] + nums[k] + nums[l] == target){
						//aloca uma linha a mais na matriz
						//aloca espaco de 4 numeros para vetor[i] ok 
						//antes de alocar precisa verificar se a quadrupla já não esta na matriz dos resultados
						//talvez usar o COmparar quadruplas, comparar sempre com a ultima inserida, se for linha,
						//nao precisa comparar com nada 
						returnColumnSizes[linha] = (int *)malloc(4 * sizeof(int));
...
//tá dando problema em algum lugar, mas onde?
void quicksort(int* nums, int inicio, int fim){
	...
	//nessa chamada o j == -1 em algum momento
	if (i < fim){
